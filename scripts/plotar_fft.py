...
while(file != ''): 
    ...
    plt.plot(a_freq, a_f)
    plt.title(comp_title+', Magnitude (M) vs. Frequência (\u03C9), Nível PWM: '+str(pwm_level))
    plt.xlabel("\u03C9 (Hz)")
    plt.ylabel("M (m/s)")
    plt.savefig("/home/gabs/Documents/Pibic/RelacoesPWM/FFT"+comp_title.split(' ')[1].replace("dos", "M")+file.split('/')[-1].replace(".txt","")+".pdf")
    plt.show()
